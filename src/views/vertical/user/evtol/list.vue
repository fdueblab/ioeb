...
export default {
  ...
  data () {
    return {
      ...
      domainArr: ['低空飞行AI应用服务'],
      industryArr: getIndustryMap('evtol'),
      scenarioArr: getScenarioMap('evtol'),
      technologyArr: getTechnologyMap('evtol'),
      containerStyle: {
        top: '0',
        left: '0'
      }
  ...
  methods: {
    ...
    async initData() {
      ...
      const [serviceData, metaData] = await Promise.all([
        getServiceData('evtol'),
        getMetaAppData('evtol')
      ])
