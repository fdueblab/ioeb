...
export default {
  ...
  data () {
    return {
      ...
      domainArr: ['跨境电商AI智能服务'],
      industryArr: getIndustryMap('ecommerce'),
      scenarioArr: getScenarioMap('ecommerce'),
      technologyArr: getTechnologyMap('ecommerce'),
      containerStyle: {
        top: '0',
        left: '0'
      }
  ...
  methods: {
    ...
    async initData() {
      ...
      const [serviceData, metaData] = await Promise.all([
        getServiceData('ecommerce'),
        getMetaAppData('ecommerce')
      ])
