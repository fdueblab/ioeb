...
export default {
  ...
  data () {
    return {
      ...
      domainArr: ['家庭陪伴AI应用服务'],
      industryArr: getIndustryMap('homeAI'),
      scenarioArr: getScenarioMap('homeAI'),
      technologyArr: getTechnologyMap('homeAI'),
      containerStyle: {
        top: '0',
        left: '0'
      }
  ...
  methods: {
    ...
    async initData() {
      ...
      const [serviceData, metaData] = await Promise.all([
        getServiceData('homeAI'),
        getMetaAppData('homeAI')
      ])
