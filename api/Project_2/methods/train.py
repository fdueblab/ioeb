...
class attHGNN(nn.Module):
    ...
    def __init__(self, in_feats, in_efeats, out_feats):
        super().__init__()
        self.linear = nn.Linear(in_feats * 2, out_feats)
        self.fc = nn.Linear(in_feats + in_efeats, in_feats)
    ...
    def message_func1(self, edges):
        node_features = edges.src['feat']  # 来源节点特征
        type_weights = edges.data['att']  # 边类型注意力
        edge_features = edges.data['edge_feat']  # 边的特征

        # 例如，可以将点特征与类型权重相乘，然后再与边特征相乘
        weighted_node_features = torch.mul(type_weights, node_features)
        combined_features = torch.cat((weighted_node_features, edge_features), dim = 1)
        messages = self.fc(combined_features)
        return {'m': messages}
    ...
    def forward(self, g, h, e_feat):
        with g.local_scope():
            ...
            for e_type, e_feature in e_feat.items():
                ...
                g.edges[e_type].data['edge_feat'] = e_feature
...
class model(nn.Module):
    def __init__(self, in_feats, in_efeats, h_feats, out_feats):
        super(model, self).__init__()
        self.conv1 = attHGNN(in_feats, in_efeats, h_feats) 
        self.conv2 = attHGNN(h_feats, in_efeats, out_feats)
...
def run_model(args):
    ...
    if args.model_type == 'atthgcn':
        ...
        net = model(feature_dim, edge_feature_dim, args.hid_dim, 3)
        labels = g.nodes['app'].data['label']
    ...
    for epoch in range(args.epoch):
        ...
        # if os.path.exists('../checkpoint/model_horizontal.pt'):
        #     print("Model checkpoint found. Stopping training.")
        #     net.load_state_dict(torch.load('../checkpoint/model_horizontal.pt'))
        #     break
        ...
        if val_loss.item() < best_val_loss:
            ...
            patience_counter = 0
            torch.save(net.state_dict(), '../checkpoint/model_horizontal.pt')
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='testing for the dataset')
    ...
    ap.add_argument('--model_type', type=str, default='atthgcn', help='hgcn or atthgcn or transhgcn(选用massfuc1)')
