...
def adj_to_tensor(adj):
    ...
    # a = 0
    # b = int(sparse_data.max())
    # for i in range(b + 1):
    #     a += torch.where(sparse_data == i)[0].shape[0]
    ...
    num_edges = sparse_data.long()
    for i in range(sparse_data.shape[0]):
        # for j in range(int(sparse_data[i])):
                # recovered_edge_index.append(sparse_concat[i])
        recovered_edge_index.append(sparse_concat[i].repeat(num_edges[i], 1))
    recovered = torch.cat(recovered_edge_index, dim=0).t()
    return recovered
...
def get_index_induc(index_a, index_b):
    ...
    Description
    -----------
    Get index under the inductive training setting. 其实就是把它规整成0-len的index
