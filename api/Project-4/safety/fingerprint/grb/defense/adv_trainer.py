...
class AdvTrainer(object):
    def __init__(self,
        ...
        class_counts = Counter(dataset.labels.numpy())
    ...
    def train(self,
        ...
        if train_mode == "inductive":
            ...
            for epoch in epoch_bar:
                ...
                elif self.loss == torch.nn.CrossEntropyLoss:
                    out = logits
                    train_loss = self.loss(weight=self.weights.to(self.device))(out, labels[train_mask.to(self.device)])
                    logits_val = model(features_val, adj_val)
                    out_val = logits_val
                    val_loss = self.loss()(out_val[val_mask_induc], labels[val_mask])
