...
def fgsm_attack(g, node_features, edge_features, labels, train_mask,val_mask, test_mask, target_model, scenario='scenario_0'):
    args = get_parser()
    ...
    if scenario == 'scenario_0':
        ...
        test_score_sur_ori, acc_list = evaluate(model, g, node_features, edge_features, labels, test_mask)
        ...
        attack = FGSM_test_simple(epsilon=args.epsilon,
                                  ...
                                  feat_lim_max=4,
                                  device=device)
        data_attack, node_features_attack, edge_features_attack, labels_sur_pred = attack.attack(model=model,
                                                     data=g,
                                                     node_features=node_features,
                                                     edge_features=edge_features,
                                                     target_mask=test_mask)
        test_score_sur_atk, acc_list_atk = evaluate(model, data_attack, node_features_attack, edge_features_attack, labels_sur_pred, test_mask)
        ...
        model_tar.eval()
        test_score_tar_ori, acc_list_tar = evaluate(model_tar, g, node_features, edge_features, labels, test_mask)
        print("Test score of target model: {:.4f}".format(test_score_tar_ori), acc_list_tar)
        print("上述结果是在ground truth上的target model的预测结果")

        test_score_tar_atk, acc_list_tar_atk = evaluate(model_tar, data_attack, node_features_attack, edge_features_attack, labels, test_mask)
