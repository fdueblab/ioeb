...
class StableTrojanNet(nn.Module):
	...
	def __init__(self, device, nfeat, nout, layernum=3, dropout=0.00):
		...
		#	if dropout > 0:
		#		layers.append(nn.Dropout(p=dropout))
		self.alpha = (torch.ones(nout * nfeat) * 0.1).to(device)
		self.beta = torch.zeros(int(nout * (nout - 1) / 2) * nout).to(device)
		# 注册参数
...
class Backdoor:
	...
	def fit(self, g, idx_attach, idx_unlabeled):
		...
		optimizer_shadow = optim.AdamW(self.shadow_model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
